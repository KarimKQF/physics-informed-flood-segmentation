...
def main() -> int:
    ...
    parser.add_argument("--max-unique-srtm-tiles", type=int, default=24)
    ...
    try:
        ...
        # Randomize the rows before selection to ensure the seed has an effect if we break ties
        random.shuffle(rows)
    ...
    print(f"[INFO] Number of selected samples: {len(selected_ids)}")
    print(f"[INFO] Unique SRTM tiles required: {len(unique_tiles)} ({unique_tiles})")

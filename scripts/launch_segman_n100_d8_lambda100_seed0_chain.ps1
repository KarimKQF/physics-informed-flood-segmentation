# SegMAN N=100 D8 downstream loss (lambda=100) -- Sequential chain launcher (seed0 only)
#
# 2 runs: D8 real DEM (lambda=100) | D8 shuffled DEM (lambda=100)
#
# Scientific context:
#   lambda=1.0 pilot gave effective D8 contribution ~0.01% (target 1-5%).
#   DEM geometry check: 72% of pixels have w>0.1 with s0=1.0m (not flat).
#   Correct fix: rescale lambda x100. tau=0.05 and s0=1.0 unchanged.
#
# SAFETY RULES:
#   - Detects any running train_segman.py and ABORTs (or kills with -KillOrphans).
#   - Never overwrites an existing completed run.
#   - Never runs CE, Dice+CE baseline, N=200, seeds 1/2, V1-topo, lambda=1 again, or parallel training.
#   - DEM is not a model input (enforced by train_segman.py itself).
#
# PREREQUISITE: Run validate_and_smoke_d8_lambda100_n100.py first and confirm all PASS.
#
# DETACHED LAUNCH (survives terminal closure):
#
#   $ROOT = "C:/flood_research/repos/physics-informed-flood-segmentation"
#   $proc = Start-Process powershell.exe `
#       -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$ROOT\scripts\launch_segman_n100_d8_lambda100_seed0_chain.ps1`"" `
#       -WindowStyle Hidden -PassThru
#   Write-Host "Chain PID: $($proc.Id)"
#
# STATUS CHECK:
#   Get-Content E:/flood_research/logs/segman_n100_d8_lambda100_seed0_master_stdout.log -Tail 30
#
# RESUME from a specific run tag:
#   -StartFrom segman_n100_d8_dem_shuffled_lambda100p0_seed0
#
...
$LOGFILE = "E:/flood_research/logs/segman_n100_d8_lambda100_seed0_master_stdout.log"
...
$variants = @(
    [pscustomobject]@{
        tag      = "segman_n100_d8_lambda100p0_seed0"
        config   = "$CFGDIR/n100_seed0_dice_ce_d8_lambda100p0.yaml"
        run_dir  = "$RUNS/segman_n100_d8_lambda100p0_seed0"
    }
    [pscustomobject]@{
        tag      = "segman_n100_d8_dem_shuffled_lambda100p0_seed0"
        config   = "$CFGDIR/n100_seed0_dice_ce_d8_dem_shuffled_lambda100p0.yaml"
        run_dir  = "$RUNS/segman_n100_d8_dem_shuffled_lambda100p0_seed0"
    }
)
...
Write-Chain "===== SegMAN N=100 D8 lambda=100 (seed0) START -- total=$($variants.Count) runs, start=$start_idx ====="
Write-Chain "Conditions: D8 real DEM (lambda=100) | D8 shuffled DEM (lambda=100, negative control)"
Write-Chain "NO CE, NO Dice+CE baseline, NO V1-topo, NO lambda=1 reruns, NO N=200, NO seeds 1/2, NO parallel training."
Write-Chain "DEM is NOT a model input. tau=0.05, s0=1.0 unchanged. Only lambda_topo=100.0 differs from pilot."
Write-Chain "Prerequisite: validate_and_smoke_d8_lambda100_n100.py PASSED."
...
for ($i = $start_idx; $i -lt $variants.Count; $i++) {
    ...
    # Safety: require lambda100 in config name
    if ($v.config -notmatch "lambda100") {
        Write-Chain "ABORT: Refusing non-lambda100 config: $($v.config)" "ERROR"
        exit 1
    }
...
Write-Chain "===== SegMAN N=100 D8 lambda=100 (seed0) COMPLETE (all $($variants.Count) runs done) ====="
Write-Chain "Next step: python experiments_cvpr/segman/aggregate_segman_n100_d8_lambda100_seed0.py"
exit 0

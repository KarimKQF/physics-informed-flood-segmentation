"""
STEP 6C/v3 5I+Physics full-data — inference-only evaluation on valid / test / Bolivia.

Loads best checkpoint (epoch 42) from the TerraMind Base + UNetDecoder + Physics run,
...
RUN_DIR = Path("E:/flood_research/experiments/terramind_baseline/runs/step6c_v3_terramind_base_unetdecoder_dice_topographic_lambda05_warmup")
CKPT_PATH = RUN_DIR / "checkpoints" / "best_checkpoint.pt"
CONFIG_PATH = RUN_DIR / "configs" / "step6c_v3_terramind_base_unetdecoder_dice_topographic_lambda05_warmup.yaml"
METRICS_OUT = RUN_DIR / "metrics" / "step6c_v3_base_unetdecoder_test_bolivia_eval.json"
REPORTS_OUT = REPO_ROOT / "reports" / "STEP_6C_V3_BASE_UNETDECODER_TEST_BOLIVIA_EVAL_SUMMARY.json"
...
def main() -> int:
    t0 = time.time()
    print("=" * 70, flush=True)
    print("STEP 6C/v3 5I+Physics full-data — inference-only eval (valid / test / Bolivia)", flush=True)
    ...
    payload = {
        "step": "6C-v3-base-unetdecoder-fulldata-eval",

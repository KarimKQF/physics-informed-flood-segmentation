"""
STEP 6C/v3 low-data N=100 — inference-only evaluation on valid / test / Bolivia.

Loads the best checkpoint from the low-data N=100 physics run, runs inference
...
Outputs (written to the run metrics directory):
    E:/.../step6c_v3_low_data_n100_seed42_lambda05_warmup/metrics/
        step6c_v3_low_data_n100_test_bolivia_eval.json

And to the project reports directory:
    reports/STEP_6C_LOW_DATA_N100_TEST_BOLIVIA_EVAL_SUMMARY.json
"""
...
# Paths
# ---------------------------------------------------------------------------
RUN_DIR = Path("E:/flood_research/experiments/terramind_baseline/runs/step6c_v3_low_data_n100_seed42_lambda05_warmup")
CKPT_PATH = RUN_DIR / "checkpoints" / "best_checkpoint.pt"
CONFIG_PATH = RUN_DIR / "configs" / "step6c_v3_low_data_n100_seed42_lambda05_warmup.yaml"
METRICS_OUT = RUN_DIR / "metrics" / "step6c_v3_low_data_n100_test_bolivia_eval.json"
REPORTS_OUT = REPO_ROOT / "reports" / "STEP_6C_LOW_DATA_N100_TEST_BOLIVIA_EVAL_SUMMARY.json"
...
def main() -> int:
    t0 = time.time()
    print("=" * 70, flush=True)
    print("STEP 6C/v3 low-data N=100 — inference-only eval (valid / test / Bolivia)", flush=True)
    ...
    payload = {
        "step": "6C-v3-low-data-n100-seed42-eval",
